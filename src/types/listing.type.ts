...
export type ListingStatusActorType = "USER" | "ADMIN" | "SYSTEM";
...
export type AdminCreateListingRequest = {
  ownerId: string;
  listing: CreateListingRequest;
};

export type ChangeListingStatusRequest = {
...
export type AdminListingDetailResponse = {
  ...
  statusHistory: ListingStatusHistoryResponse[];
};

export type AdminListingSummaryResponse = {
  id: string;
  title: string;
  category: ListingCategory;
  subtype: ListingSubtype;
...
export interface AdminListingQueryParams {
  page?: number;
  size?: number;
  status?: ListingStatus;
  keyword?: string;
  ...
  sort?: string;     // e.g. "submittedAt,desc"
}
